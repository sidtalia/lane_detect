...
class lane_driver:
  def __init__(self):
    ...
    sub_odom = rospy.Subscriber("/zed2/zed_node/odom", Odometry, self.odom_callback, queue_size = 1)
  ...
  def odom_callback(self, msg):
    ...
    # print(self.rpy*57.3)
    self.inferred_yaw = self.rpy[2] #self.inferred_yaw + self.rotBF[2]*0.04 ## 50 hz
  ...
  def project_bodyframe_to_cam(self, x, y):
    ...
    xl, yl = self.XY2img(x_, y_, self.height, self.K_v, self.K_h,
                    self.ox*2, self.oy*2, self.ox, self.oy, -0.04, 512)
  ...
  def lane_stuff(self, x, y, xw, yw, th):
    ...
    last_x = last_x[-100:]
    last_y = last_y[-100:]  # limit number of points
    ...
    self.last_pos = np.array([xw, yw])
    # self.point_culling(100, xw, yw)  # TODO list scale this with speed

  # Define a callback for the Image message
  def image_callback(self, img_msg):
    if(time.time() - self.last_inference < self.expected_frame_time):
      return
    self.last_inference = time.time()
    global DEBUG
